...
document.getElementById("company_clauses").textContent = params.get("company_clauses");
...
const rest = (Number(total) - Number(advance));
...
equipments.forEach(element => {
    ...
    const programsCount = element.listPrograms.length;
    const obsCount = element.observations.length;
    const checkListCount = element.cheklist.length;

    const trDevice = document.createElement("tr");
    trDevice.id = "trDevice";

    const tr = document.createElement("tr");

    const trObs = document.createElement("tr");
    trObs.id = "trObs";
    const trObsLabel = document.createElement("tr");
    trObsLabel.id = "trObsLabel";

    const trProgramsLabel = document.createElement("tr");
    trProgramsLabel.id = "trProgramsLabel";
    const trPrograms = document.createElement("tr");
    trPrograms.id = "trPrograms";

    const trCheckListLabel = document.createElement("tr");
    trCheckListLabel.id = "trCheckListLabel";
    const trCheckList = document.createElement("tr");
    trCheckList.id = "trCheckList";
    ...
    // LABEL OBSERVACIONES
    const tdObsLabel = document.createElement("td");
    tdObsLabel.textContent = 'OBSERVACIONES:';
    tdObsLabel.colSpan = 2;
    trObsLabel.appendChild(tdObsLabel);

    // OBSERVACIONES
    const tdObservations = document.createElement("td");
    tdObservations.textContent = element.observations || "";
    tdObservations.colSpan = 2;
    trObs.appendChild(tdObservations);


    // CHECK LIST LABEL
    const tdChecklistLabel = document.createElement("td");
    tdChecklistLabel.textContent = 'ACCESORIOS:';
    tdChecklistLabel.colSpan = 2;
    trCheckListLabel.appendChild(tdChecklistLabel);

    // CHECKLISTS
    const tdChecklist = document.createElement("td");
    tdChecklist.colSpan = 2;
    ...
    trCheckList.appendChild(tdChecklist);


    tbody.appendChild(tr);

    // PROGRAMAS LABEL
    const tdListProgramsLabel = document.createElement("td");
    tdListProgramsLabel.textContent = 'PROGRAMAS:';
    tdListProgramsLabel.colSpan = 2;
    trProgramsLabel.appendChild(tdListProgramsLabel);
    ...
    tdListPrograms.colSpan = 2;
    trPrograms.appendChild(tdListPrograms);
    ...
    if (checkListCount > 0) {
        tbody.appendChild(trCheckListLabel);
        tbody.appendChild(trCheckList);
    }
    // Agregar observaciones si existe
    if (obsCount > 0) {
        tbody.appendChild(trObsLabel);
        tbody.appendChild(trObs);
    }
    // Agregar programas si existe
    if (programsCount > 0) {
        tbody.appendChild(trProgramsLabel);
        tbody.appendChild(trPrograms);
    }
